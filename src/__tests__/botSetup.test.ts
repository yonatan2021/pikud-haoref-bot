...
describe('setupBotHandlers', () => {
  it('registers all expected commands', async () => {
    ...
    const expected = ['start', 'profile', 'add', 'zones', 'mycities', 'settings', 'stats', 'history', 'connect', 'contacts'];
    assert.equal(bot._registeredCommands.length, expected.length, 'should register exactly 10 commands');
  ...
  it('calls bot.api.setMyCommands with all 8 commands', async () => {
    ...
    assert.equal(cmds.length, 10, 'setMyCommands should receive exactly 10 commands');

    const commandNames = cmds.map((c) => c.command);
    for (const name of ['start', 'profile', 'add', 'zones', 'mycities', 'settings', 'stats', 'history', 'connect', 'contacts']) {
