...
type TypeFilter = 'all' | 'group' | 'newsletter';
...
export function SourceSelector({ chats, value, onChange, disabled, whatsappConnected }: SourceSelectorProps) {
  ...
  // Total counts (before search, for tab badges)
  const totalCounts = useMemo(() => {
    const all = chats ?? [];
    return {
      groups: all.filter(c => c.type === 'group').length,
      channels: all.filter(c => c.type === 'newsletter').length,
    };
  }, [chats]);
  ...
  const { groups, channels } = useMemo(() => {
    ...
    return {
      groups: filter === 'newsletter' ? [] : filtered.filter(c => c.type === 'group'),
      channels: filter === 'group' ? [] : filtered.filter(c => c.type === 'newsletter'),
    };
  }, [chats, search, filter]);
  ...
  const tabs: { key: TypeFilter; label: string; count: number }[] = [
    ...
    { key: 'newsletter', label: 'ערוצים', count: totalCounts.channels },
  ];
