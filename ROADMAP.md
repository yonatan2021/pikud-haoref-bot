...
### v0.4.4 — Smart Zone Visualization (Cognitive Accessibility) ⭐
- **צבע ייחודי ל-28 zones**: כל zone בצבע קבוע ועקבי — על המפה (Mapbox פוליגונים) ובטקסט (אימוג'י super-region)
- **אימוג'י דחיפות לפי countdown**: 🔴 0-15 שניות (מיידי), 🟠 15-30 (דחוף), 🟡 30-60 (מהיר), 🟢 60-90 (מתון), 🔵 90+ (רחוק) — מוחלף על ה-emoji הגנרי בכותרת zone
- **מיון לפי דחיפות**: zones ממוינים מה-countdown הנמוך ביותר (הכי דחוף) למעלה — גם בערוץ וגם ב-DM
- **Summary line מובנה**: "3 אזורים · 12 ערים · ⏱ 15-90 שניות" — סיכום שנקלט ברפרוף
- **מפרידים ויזואליים**: קו ברור בין zones (לא blob רצוף), רווח + header ברור
- **Zone headers מועשרים**: `🟠 עוטף עזה (4) · ⏱ 15 שניות` — צבע + שם + ספירה + countdown
- **מקרא צבעים (legend)** על המפה: פינת מפה עם 6 super-regions וצבעיהם
- **גבולות zones על המפה**: קווי הפרדה בין אזורים שכנים שמותרעים
- **נגישות קוגנטיבית נוספת**: ספירה מספרית ברורה (לא רק רשימת שמות), עקביות צבעים בכל הודעה, progressive disclosure (סיכום → פרטים)
