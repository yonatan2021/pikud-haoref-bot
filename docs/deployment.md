...
אם אתה רוצה לפתוח את הדשבורד ממכשיר אחר באותה רשת:

הדשבורד וה-health מאזינים עכשיו על `0.0.0.0`, כך שמכשירים באותה רשת יכולים
להגיע אליהם דרך IP של ה-Mac.

1. מצא את כתובת ה-IP המקומית של ה-Mac:

   ```bash
   ipconfig getifaddr en0
   ```

2. פתח מהטלפון `http://<mac-ip>:4000/dashboard`
3. אם זה לא עובד, בדוק `System Settings → Network → Firewall` וודא ש-`Node`
   או `tsx` מורשים לחיבורים נכנסים
4. אם גם זה לא פותר, רק אז שקול לשנות את `listen` ל-`0.0.0.0`
